	...
	for (let i = 1; i <= totalPeople; i++) {
		...
		if (i != totalPeople ) {
			...
			if(endShlokaNumber == mahaMantras) {
				...
				if(totalPeople - counter < 20){
					perpersonShlokasDecimal = 38/(totalPeople-counter);
				}
				counter++;
				makeText = makeText+fillDahses25("Nyasa: ") + curatedLines[counter] + '\n';
				strCsv = strCsv + 'Nyasa' + ',,,,' + curatedLines[counter] + "\n\n";
				counter++;
				makeText = makeText+ assignDhyaanam(peopleForDhyanam, counter, curatedLines);
	            strCsv = strCsv + '\n';	
				}
